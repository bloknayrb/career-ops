...
const scripts = [
  ...
  // tracker-columns-tests.mjs and tracker-writer-lock-tests.mjs above (#2906).
  // This one crossed the 30s default at v1.32.0 and was killed as
  // `exit null, signal SIGTERM` while passing when run alone. Not contention:
  // measured UNLOADED on this box well past the default budget.
...
try {
  ...
  // FORK-LOCAL: derive the expected margin from the same constant the renderer
  // uses. Upstream hardcodes '0.6in' in the two assertions below; a third copy
  // of that literal is how the margin drifted out of sync twice. This fork keeps
  // it in pdf-config.mjs so generate-pdf and generate-cover-letter cannot
  // disagree, and the test reads it from there.
  ...
  if (
    injectedPageCss.includes(`@page { size: Letter; margin: var(--page-margin, ${PDF_PAGE_MARGIN}); }`) &&
  ...
  if (mixedCasePageCss.includes(`@page { size: Letter; margin: var(--page-margin, ${PDF_PAGE_MARGIN}); }`)) {
